...
export default function AdminPanel() {
  ...
  return (
    <div className="page-container animate-fade-in">
      ...
      {/* Tab Bar */}
      <div className="flex gap-1 bg-slate-100 rounded-xl p-1.5 mb-8 w-fit shadow-inner">
        {tabs.map(tab => (
      ...
      <div className="flex gap-3 flex-wrap items-center">
        <div className="relative">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="搜索用户名或邮箱..."
            className="input-focus pl-10 pr-4 py-2.5 border border-slate-200 rounded-xl text-sm w-72 bg-slate-50 placeholder:text-slate-400" />
        </div>
        <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)}
          className="border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none bg-slate-50
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center animate-fade-in">
          ...
          <div className="relative bg-white rounded-2xl p-6 w-full max-w-sm mx-4 shadow-card-elevated animate-scale-in">
            ...
            <button onClick={() => setSelectedUser(null)}
              className="w-full mt-5 bg-slate-100 text-slate-600 py-2.5 rounded-xl text-sm font-semibold
...
function QuestionsTab() {
  ...
  return (
    <div className="animate-fade-in space-y-5">
      ...
      <div className="flex gap-3 items-center flex-wrap">
        <div className="relative">
          ...
          <input value={search} onChange={e => { setSearch(e.target.value); if (e.target.value) { setExpandedBank(null); setExpandedLevel(null); } }}
            ...
            className="input-focus pl-10 pr-4 py-2.5 border border-slate-200 rounded-xl text-sm w-80 bg-slate-50 placeholder:text-slate-400" />
        </div>
      ...
      {search.trim() ? (
        <div className="card overflow-hidden">
          <div className="px-5 py-3 bg-slate-50 border-b text-xs text-slate-500 font-medium">
            搜索结果：
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex gap-1.5 mt-2 flex-wrap">
                          ...
                          <span className="px-2 py-0.5 bg-slate-100 text-slate-500 rounded-md text-[10px]">{bank.positionName}</span>
                          {q.tags.map(t => (
                            <span key={t} className="px-2 py-0.5 bg-slate-100 text-slate-500 rounded-md text-[10px]">{t}</span>
                          ))}
                        </div>
                      </div>
          ...
          <div className="space-y-3">
            {inProgress.map(item => (
              <div key={item.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl
                                             hover:bg-slate-100/60 transition-colors card-clickable border border-transparent">
