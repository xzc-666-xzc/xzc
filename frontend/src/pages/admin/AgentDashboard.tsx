...
export default function AgentDashboard({ onBack }: { onBack: () => void }) {
  ...
  const [showDispatchConfig, setShowDispatchConfig] = useState(false);
  const [dispatchConfigs, setDispatchConfigs] = useState<Array<{ workOrderType: string; assigneeId: string; assigneeName: string }>>([]);
  const [dispatchAdmins, setDispatchAdmins] = useState<Array<{ id: string; realName: string; username: string }>>([]);
  ...
  const fetchDispatchConfig = async () => {
    ...
    try {
      ...
      setDispatchConfigs((cfgRes.data?.data as any[]) || []);
      setDispatchAdmins((adminRes.data?.data as any[]) || []);
    } catch {} finally { setDispatchLoading(false); }
  };

  const handleDispatchChange = async (workOrderType: string, assigneeId: number) => {
    try {
      await workOrderService.updateDispatchConfig(workOrderType, assigneeId);
      setDispatchConfigs(prev => prev.map(c =>
        c.workOrderType === workOrderType ? { ...c, assigneeId: String(assigneeId), assigneeName: dispatchAdmins.find(a => a.id === String(assigneeId))?.realName || '' } : c
      ));
    } catch { alert('更新失败'); }
  };
    ...
    <div className="h-[calc(100vh-4rem)] flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-white border-b border-slate-100 shrink-0">
        <div className="flex items-center gap-4">
          ...
          {isGxzc && (
            <button
              onClick={() => { setShowDispatchConfig(!showDispatchConfig); if (!showDispatchConfig) fetchDispatchConfig(); }}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                showDispatchConfig ? 'bg-accent-100 text-accent-700' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'
              }`}>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3.5 h-3.5"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>
              {showDispatchConfig ? '收起配置' : '分发配置'}
            </button>
          )}
        </div>
      ...
      {/* ===== Dispatch Config Panel (Gxzc only) ===== */}
      {showDispatchConfig && isGxzc && (
        <div className="px-4 py-4 bg-white border-b border-slate-100 shrink-0 animate-fade-in">
          <h4 className="text-sm font-semibold text-ink-title mb-3 flex items-center gap-2">
            <span className="w-1.5 h-4 rounded-full bg-brand-500" />📋 工单分发配置
          </h4>
          {dispatchLoading ? (
            <div className="flex justify-center py-4"><div className="w-5 h-5 border-2 border-accent-500 border-t-transparent rounded-full animate-spin" /></div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {dispatchConfigs.map(cfg => (
                <div key={cfg.workOrderType} className="bg-cool-alt rounded-xl p-3">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-xs font-semibold text-ink-title">
                      {{ 'INTERVIEW_FAULT': '⚠️ 面试故障', 'FEATURE_SUGGESTION': '💡 功能建议', 'BUG_REPORT': '🐛 BUG上报' }[cfg.workOrderType] || cfg.workOrderType}
                    </span>
                  </div>
                  <select
                    value={cfg.assigneeId}
                    onChange={(e) => handleDispatchChange(cfg.workOrderType, Number(e.target.value))}
                    className="w-full px-3 py-2 rounded-lg border text-sm outline-none bg-white focus:border-accent-500 transition-all"
                    style={{ borderColor: 'var(--border-light)' }}>
                    {dispatchAdmins.map(a => (
                      <option key={a.id} value={a.id}>{a.realName} ({a.username})</option>
                    ))}
                  </select>
                  {cfg.assigneeName && (
                    <p className="text-[10px] text-ink-muted mt-1.5">当前：{cfg.assigneeName}</p>
                  )}
                </div>
              ))}
            </div>
